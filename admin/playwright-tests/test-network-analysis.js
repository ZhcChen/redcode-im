...
(async () => {
  ...
  try {
    ...
    // 按照用户描述的点击流程执行操作
    console.log('开始执行点击流程...');

    // 点击第一个div (聊天记录菜单)
    await page.click('div.arco-menu-inline-header.arco-menu-has-icon');
    await page.waitForTimeout(1000);

    // 点击第二个div (聊天记录列表)
    await page.click('div.arco-menu-item');
    await page.waitForTimeout(1000);

    // 点击第二个button (查看用户记录)
    await page.click('button.arco-btn-text');
    await page.waitForTimeout(1000);

    // 点击第三个div (用户消息标签)
    await page.click('div.arco-tabs-tab');
    await page.waitForTimeout(3000);

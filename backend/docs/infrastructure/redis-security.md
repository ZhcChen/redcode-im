...
command: |
  redis-server
  --port 6381
  --requirepass ${REDIS_SESSION_PASSWORD:-redis_session_password}
  # ... 其他配置
```
...
```bash
# 使用 redis-cli 连接
redis-cli -p 6381 -a redis_session_password
...
```bash
# 在 Docker 容器中测试
docker exec redcode-redis-session redis-cli -p 6381 -a $REDIS_SESSION_PASSWORD ping
```

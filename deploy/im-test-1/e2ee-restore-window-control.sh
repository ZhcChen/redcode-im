...
for path in "$env_file" "$drill_compose" "$restore_control" "$backup_control" "$artifact_root"; do
...
cleanup() {
  local exit_code="${1:-0}" containers volumes owner
  owner="$(current_owner)"
  ...
  rm -f "$artifact_dir/database.dump" "$artifact_dir/database-corrupt.dump" "$artifact_dir/report.json"
...
switch_to_restore() {
  ...
  E2EE_RESTORE_ALLOW_PREPARE=yes E2EE_RESTORE_DUMP_PATH="$dump_path" \
...
case "$mode" in
  ...
  cleanup) cleanup ;;
  *)
    log "用法：e2ee-restore-window-control.sh candidate-prepare|switch|verify|cleanup"
    exit 64
    ;;
esac

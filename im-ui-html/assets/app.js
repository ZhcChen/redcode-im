(function bootstrapIMPrototype() {
  ...
  function renderChatListScreen() {
    ...
    return `
      <section class="screen screen--tabbed runtime-screen runtime-screen--list runtime-screen--chat-list ${state.conversationMenuChatId ? "has-overlay" : ""}">
        ${renderScreenHeader({
          title: "聊天",
          root: true,
          actions: [
  ...
  function renderConversationActionSheet() {
    ...
    return `
      <div class="runtime-conversation-menu-layer">
        ...
        <section class="runtime-conversation-menu" style="--conversation-menu-left: ${anchor.left}px; --conversation-menu-top: ${anchor.top}px;" role="dialog" aria-modal="true" aria-label="${escapeHtml(chat.name)} 会话操作">
          ...
          ${[["mentions", "仅提及"], ["muted", "静音"]].map(([value, label]) => `<button class="${notificationMode === value ? "is-active" : ""}" type="button" data-action="set-conversation-notification" data-chat-id="${chat.id}" data-notification-mode="${value}">${label}</button>`).join("")}
  ...
  function sortedChats() {
    return data.chats.filter((chat) => !chat.archived).sort((left, right) => {
          ...
          .map(
            (toast) => `
              <div class="toast">
                ...
                ${toast.action ? `<button type="button" data-action="perform-toast-action" data-toast-id="${toast.id}">${escapeHtml(toast.action.label)}</button>` : ""}
              </div>
            `,
          )
          .join("")}
  ...
  function performToastAction(toastId) {
    const toast = state.toasts.find((item) => item.id === toastId);
    if (!toast?.action) return;
    if (toast.action.name === "restore-conversation") {
      const chat = findChat(toast.action.payload?.chatId);
      if (new URLSearchParams(window.location.search).get("state") === "restore-error") {
        toast.title = "恢复失败";
        toast.message = "网络连接不稳定，请重试。";
        toast.action.label = "重试";
        render();
        return;
      }
      if (!chat) {
        toast.title = "恢复失败";
        toast.message = "会话不存在，请稍后重试。";
        toast.action = null;
        render();
        return;
      }
      ...
      showToast("会话已恢复", `${chat.name} 已回到原来的位置。`);
      render();
    }
  }
  ...
  function handleKeydown(event) {
    ...
    if (
      event.key === "Escape" &&
      (state.contactActionSheetContactId || state.contactRemarkEditorContactId || state.settingsSheetKey || state.actionDialog || state.messageMenu || state.messageEditor || state.chatActionsChatId || state.conversationMenuChatId)
    ) {
      event.preventDefault();
      if (state.actionDialog) {
        closeActionDialog();
  ...
  function handleClick(event) {
    ...
    if (action === "set-conversation-notification") {
      const chat = findChat(target.getAttribute("data-chat-id"));
      const requestedMode = target.getAttribute("data-notification-mode");
      if (chat && ["mentions", "muted"].includes(requestedMode)) {
        const currentMode = chat.notificationMode || (chat.muted ? "muted" : "all");
        const nextMode = currentMode === requestedMode ? "all" : requestedMode;
        chat.notificationMode = nextMode;
        chat.muted = nextMode === "muted";
        state.conversationMenuChatId = null;
        state.conversationMenuAnchor = null;
        showToast("通知设置已更新", `${chat.name}：${nextMode === "all" ? "已恢复默认通知" : nextMode === "mentions" ? "仅提及" : "静音"}。`);
        render();
      }
      return;
    }
    if (action === "archive-conversation") {
      archiveConversation(target.getAttribute("data-chat-id"));
      return;
    }
    if (action === "perform-toast-action") {

...
- 固定页头下的收到/发出分类栏；Tab 不随申请列表滚动，两个等宽分类由细分隔线组织，当前分类以左侧状态条与强调数量标识
- 单一活动面板：仅展示当前分类的申请列表
- 关系请求摘要：头像、身份与右上角时间构成顶行；打招呼使用整卡宽度的摘要行；底部动作不使用额外分割线

备注：

- “收到的申请”与“发出的申请”使用带当前状态条的固定顶部分类栏切换，不再纵向拼接两个分组；Tab 内数字说明当前分类的总量。
- 每条申请使用关系摘要卡：时间位于右上角，打招呼跨整卡宽度呈现，动作区以留白而非横线分隔。

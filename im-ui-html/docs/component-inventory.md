...
- 固定页头下的收到/发出分类 Tab；Tab 不随申请列表滚动，使用无外轨的轻量导航，Tab 内显示各分类总数
- 单一活动面板：仅展示当前分类的申请列表
- 紧凑身份区：头像、姓名、账号/岗位/时间与单一状态标签
- 最多两行的打招呼摘要，不再嵌套内容卡或重复显示状态标题
- 收到申请时提供底部分栏的通过/忽略决策；其他状态按原位置保留

备注：

- “收到的申请”与“发出的申请”使用无外轨的固定顶部分类 Tab 切换，不再纵向拼接两个分组；Tab 内数字说明当前分类的总量。
- 申请卡和空状态复用运行时 Surface 容器的圆角，而不使用低圆角特例。
- Tab 必须使用按钮语义、当前状态和对应面板关联，并支持键盘方向键切换。
- 每条申请是独立决策单元，必须由单独表面卡片承载，不能将多条申请塞进一个连续的大容器；卡片内部不再嵌套打招呼内容卡。

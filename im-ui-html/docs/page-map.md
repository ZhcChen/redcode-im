...
| Route | 说明 |
| --- | --- |
| `#/chat/:chatId` | 聊天详情；置顶消息在会话头部下方显示单行摘要，消息流在其下独立滚动 |
...
| `#/groups/settings/:groupId` | 群设置：复合头像群概览后展示成员预览，并按角色提供群名称、群介绍、头像、权限与生命周期操作；不存在独立群公告字段 |
| `#/groups/:groupId/members` | 群成员：搜索、长名单、角色标识、成员资料入口和按角色移除 |
| `#/groups/:groupId/admins` | 群管理员：群主与管理员权限摘要、任命/移除和群主转让 |
| `#/groups/:groupId/join-requests` | 入群申请：待处理、已同意、已拒绝状态，管理员可审核 |
| `#/groups/:groupId/invite` | 创建群邀请：搜索和多选联系人；不提供邀请列表 |
| `#/groups/:groupId/invitations/:invitationId` | 按已知邀请 ID 查看并响应待处理、已接受、已拒绝或已过期邀请 |
| `#/groups/:groupId/rules` | 群规则：列表、创建、删除、排序与空态 |
| `#/groups/:groupId/mutes` | 禁言管理：全员禁言、成员定时禁言和解除 |
| `#/groups/:groupId/operation-logs` | 操作日志：仅按 `limit/offset` 分页，不提供条件筛选 |

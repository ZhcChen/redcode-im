...
| `Radius` | `8 / 10 / 12 / 16 / Pill` | 业务列表与卡片不超过 `8px`；输入与操作控件使用 `10px`；Sheet 可使用 `16px` |
...
- 激活态优先用主色、细指示与弱高亮表达，不依赖大面积胶囊底、复杂填充或插画。
- 当前正式 key 统一收敛为三组：
...
- `#/mobile-design`：独立手机设备画布，默认展示聊天首页；其业务路径以 `#/mobile-design/<业务路径>` 保留在同一手机容器内。
- `#/mobile-design` 是独立设备预览模式：不显示工具条、说明卡或验证侧栏，只显示一个包含完整 IM 交互的手机容器。
- 设计评审模式：普通业务路由在桌面默认显示机框与评审控制；可显式通过 `?mode=review` 固定。
- 手机运行模式：普通业务路由在 `640px` 以下直接显示应用界面，不显示外层工具条、机框或验证卡；可通过 `?mode=app` 强制开启。

### 业务壳层

- 移动端一级导航：聊天 / 联系人 / 发现 / 设置，采用稳定底栏，不使用覆盖内容的大型浮动胶囊。
- 所有详情、搜索、申请、群设置下沉到二级页。
- 聊天详情的消息流优先级最高；公告、资料、文件与设置以轻提示或二级入口承载。

...
> 冻结版本：`2026-08-01`。token 变更必须同步组件清单、多端 handoff 和受影响的预览回归，不允许正式端自行派生另一套视觉值。
...
| `Surface Soft` | `#EEF3FA` | 输入区、轻提示，以及 Quiet Icon 的按下反馈；不作为顶部工具按钮的默认底色 |
...
| `Control Height` | `40 / 44 / 48` | 聊天 composer `40px`；固定导航内搜索 `44px`；页面内搜索与单行表单字段 `48px` |
...
- `#/mobile-design`：独立手机设备画布，默认展示聊天首页；宽屏默认使用 `iPhone 12 Pro` 外壳，可切换 `iPhone 16 Pro Max` 或 `Pixel 8 Pro`，并可在 `演示数据` 与 `空数据` 两个场景间切换；其业务路径以 `#/mobile-design/<业务路径>` 或 `#/mobile-design/empty/<业务路径>` 保留在同一手机容器内。
- `#/mobile-design` 是独立设备预览模式：不显示工具条、说明卡或验证侧栏；设备和数据场景切换器仅在宽屏作为紧凑预览控制存在，App 内容、导航和 Toast 必须受独立屏幕裁切层约束。空数据场景仅清空业务内容，保留已登录账号、设置和静态服务入口以审查真实空态。
...
- 聊天详情的消息流优先级最高；置顶消息以头部下方固定的单行上下文条呈现，资料、文件与其他低频信息以轻提示或二级入口承载。

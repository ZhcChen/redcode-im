...
final class StorageTests: XCTestCase {
    ...
    func testMessageSearchIndexStrategyPrefersInMemoryUntilThreshold() {
        let strategy = MessageSearchIndexStrategy(sqliteFTS5Threshold: 100)

        XCTAssertEqual(strategy.backendForIndexedMessageCount(99), .inMemory)
    ...
    @MainActor
    func testGRDBChatSummaryCacheStoresSortsAndRemovesChats() throws {
        let container = try RedCodeDatabase.makeDatabase(inMemory: true)
        let store = GRDBChatSummaryCacheStore(database: container)
    ...
    @MainActor
    func testGRDBContactCacheStoreStoresSortsUpsertsRemovesAndClearsContacts() throws {
        let container = try RedCodeDatabase.makeDatabase(inMemory: true)
        let store = GRDBContactCacheStore(database: container)
    ...
    @MainActor
    func testGRDBGroupCacheStoreStoresUpsertsRemovesAndClearsGroups() throws {
        let container = try RedCodeDatabase.makeDatabase(inMemory: true)
        let store = GRDBGroupCacheStore(database: container)
        let baseDate = Date(timeIntervalSince1970: 1_000)
    ...
    @MainActor
    func testGRDBSchemaStoresCoreCacheRecords() throws {
        ...
        try database.dbQueue.write { db in
            ...
            try RedCodeAppConfigRecord(
                key: "message_runtime",
                valueJSON: #"{"server_storage_mode":"persist"}"#
            ).insert(db)
        }

        let chats = try database.dbQueue.read { db in try RedCodeChatRecord.fetchAll(db) }
        let contacts = try database.dbQueue.read { db in try RedCodeContactRecord.fetchAll(db) }
        let groups = try database.dbQueue.read { db in try RedCodeGroupRecord.fetchAll(db) }
        let configs = try database.dbQueue.read { db in try RedCodeAppConfigRecord.fetchAll(db) }
    ...
    @MainActor
    func testGRDBAppConfigStoreSavesUpdatesRemovesAndClearsValues() throws {
        let container = try RedCodeDatabase.makeDatabase(inMemory: true)
        let store = GRDBAppConfigStore(database: container)
    ...
    @MainActor
    func testGRDBMessageCacheStoreKeepsNewestMessagesPerRoom() throws {
        let container = try RedCodeDatabase.makeDatabase(inMemory: true)
        let store = GRDBMessageCacheStore(
            database: container,
    ...
    @MainActor
    func testGRDBMessageCacheStoreClearsRoomMessages() throws {
        let container = try RedCodeDatabase.makeDatabase(inMemory: true)
        let store = GRDBMessageCacheStore(database: container)
    ...
    @MainActor
    func testGRDBMessageSearchStoreRebuildsFiltersAndPaginatesLocalIndex() throws {
        let container = try RedCodeDatabase.makeDatabase(inMemory: true)
        let cache = GRDBMessageCacheStore(database: container)
        let searchStore = GRDBMessageSearchStore(messageCacheStore: cache)
        let base = Date(timeIntervalSince1970: 1_000)

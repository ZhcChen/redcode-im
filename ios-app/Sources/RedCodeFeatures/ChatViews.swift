...
public struct ChatHomeView: View {
    ...
    @StateObject private var listController: ChatListController
    ...
    public init(
        ...
        notificationNavigation: NotificationNavigationController
    ) {
        ...
        _contactsController = ObservedObject(wrappedValue: contactsController)
        ...
        _notificationNavigation = ObservedObject(wrappedValue: notificationNavigation)
        _listController = StateObject(wrappedValue: listController)
    }
    ...
    public var body: some View {
        ...
        .onChange(of: notificationNavigation.pendingDestination?.id) { _ in
            handlePendingNotificationDestination()
        }
        .redcodeNavigationDestination(item: $notificationChatRoute) { route in
            ChatDetailView(
                authController: authController,
...
struct ChatDetailView: View {
    ...
    private let realtimeController: ChatRealtimeController
    @ObservedObject private var chatListController: ChatListController
    private let makeGroupManagementController: (@MainActor () -> GroupManagementController)?
    @ObservedObject private var contactsController: ContactsController
    private let mediaAPI: (any MediaAPIService)?
    private let emojiAPI: (any EmojiAPIService)?
    ...
    @StateObject private var controller: ChatDetailController
    @State private var draftText = ""
    @State private var showEmojiPalette = false
    @State private var backgroundPreference: ChatBackgroundPreference = .default
    @State private var stickerController: EmojiStickerController? = nil
    ...
    init(
        ...
        self.chat = chat
        self.realtimeController = realtimeController
        _chatListController = ObservedObject(wrappedValue: chatListController)
        self.makeGroupManagementController = makeGroupManagementController
        _contactsController = ObservedObject(wrappedValue: contactsController)
        self.mediaAPI = mediaAPI
        self.emojiAPI = emojiAPI
            ...
            ToolbarItem(placement: .primaryAction) {
                if let makeMessageSearchController {
                    NavigationLink {
                        MessageSearchView(
                            authController: authController,
                            chats: chatListController.chats,
        ...
        Group {
            if controller.messages.isEmpty && !controller.isLoading {
                ...
                ScrollViewReader { proxy in
                    ...
                    .listStyle(.plain)
                    .redcodeScrollContentBackgroundHidden()
    ...
    private var composer: some View {
        VStack(spacing: 8) {
            ...
            HStack(alignment: .bottom, spacing: 8) {
                ...
                .sheet(isPresented: $isPhotoPickerPresented) {
                    PhotoPickerView(selectionLimit: 6) { results in
                        Task {
                            await handlePhotoPickerResults(results)
                        }
                    }
                }
                #endif
        ...
        .overlay(alignment: .topLeading) {
            if showEmojiPalette, let stickerController {
        ...
        .onChange(of: showEmojiPalette) { isShown in
            if isShown {
    ...
    #if canImport(UIKit)
    private func handlePhotoPickerResults(_ results: [PHPickerResult]) async {
        ...
        for result in results {
            ...
            do {
                ...
                let kind: PreparedUploadKind = contentType.conforms(to: .movie) ? .video : .image
                let prepared = try MediaUploadPreparer.prepareFile(
                    at: url,
                    kind: kind,
                    fileName: fileName,
...
private struct ChatComposerEmojiPanel: View {
    @ObservedObject private var stickerController: EmojiStickerController
    let token: String?
    ...
    init(
        ...
        token: String?,
        onEmojiSelect: @escaping (String) -> Void,
        onStickerSelect: @escaping (EmojiItem) -> Void
    ) {
        _stickerController = ObservedObject(wrappedValue: stickerController)
        self.token = token
        self.onEmojiSelect = onEmojiSelect
        self.onStickerSelect = onStickerSelect
    }
...
private struct StickerItemButton: View {
    ...
    init(
        ...
        token: String?,
        stickerController: EmojiStickerController,
        onSelect: @escaping (EmojiItem) -> Void
    ) {
        self.item = item
        self.token = token
        _stickerController = ObservedObject(wrappedValue: stickerController)
...
#if canImport(UIKit)
/// iOS 15 兼容的照片/视频选择器（PHPicker，iOS 14+）。

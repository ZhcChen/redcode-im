...
SwiftUI App Shell
  ...
  -> API / WebSocket / GRDB(SQLite) / Keychain
```

### App

- 放置 SwiftUI `App` 入口、环境注入、根导航和登录态切换。
- iOS 16+ 使用 `NavigationStack` 管理页面导航；iOS 15 通过
  `AppNavigationContainer` 回退到 `NavigationView`，保证最低系统可编译。
...
- 消息、会话、联系人、群资料、App 配置缓存使用 GRDB/SQLite
  （`RedCodeDatabase` + record 模型），最低系统 iOS 15，不依赖 SwiftData。
- 消息全文搜索当前使用本地内存索引（`GRDBMessageSearchStore`），
  后续可切换 SQLite FTS5/GRDB 虚拟表，不替代主缓存模型。
...
- ViewModel 统一使用 `ObservableObject` + `@Published`（兼容 iOS 15），
  View 通过 `@StateObject` / `@ObservedObject` 订阅。
...
- 最低系统 iOS 15 下 SwiftData 不可用（iOS 17+ 专属）。
- 原生 SQLite 性能好，支持事务、索引、FTS5 全文搜索和 schema 迁移。
- 适合会话、消息、联系人、配置等结构化本地缓存。

- FTS5 全文搜索索引作为消息量增长后的后续增强，当前内存索引已覆盖
  每房间最近 200 条缓存的搜索场景。
...
- Flutter 的 service、storage、model、test 是行为参考；iOS 端落地为 Swift package、ViewModel、Repository、Actor/Service、GRDB/Keychain/FileManager。

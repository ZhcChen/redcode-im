...
> [!NOTE]
> 本文件为原生迁移历史任务树，保留原始 SwiftData 相关表述作为阶段记录。
> 2026-08-04 起客户端主线为原生双端（`android-app` + `ios-app`），本地存储已
> 切换为 GRDB/SQLite（见 `docs/plans/2026-08-04-005`），后续工作以原生客户端
> 重建计划为准，不再以本文 SwiftData 描述作为当前技术选型依据。

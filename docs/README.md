...
- **桌面端头像直传稳定**：TypeScript 端补充 `client_debug` 日志、Rust HTTP 透传 `injectToken/forceStreaming`，并内置 `VITE_AUTO_UPLOAD_TEST` 自动化脚本，可在 30s 内完成登录+上传+日志采集（详见 `docs/file_upload_troubleshooting.md`）。
...
- `docs/handbook/`：团队规范与代理说明，例如 `AGENTS.md`。
- `docs/notion/`：Notion 同步脚本与导入记录（`API_IMPORT_SUMMARY.md`、`NOTION_API_UPDATE_GUIDE.md` 等）。
- `docs/reports/`：项目分析、任务清单等交付文档。
- `docs/*.md`：现有技术方案（E2EE、文件上传、桌面残留任务等）。

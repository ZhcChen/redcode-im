...
- 当前主线：原生双端 E2EE 客户端专项已完成 -> 关闭 E2EE 生产发布剩余两个 P0
  -> 原生功能迁移总收口 -> 多平台发布。
- 当前立即任务：`G2.2-G2.3` 六端门禁与 16 个负向夹具已通过；从 `G2.4` 接入 CI/release 阻断，随后以 `G2.5` 关闭 U7 P0-3。
...
- [ ] `IM2-U10` E2EE 发布门禁
  ...
  - G1 已完成隔离候选三端 live、备份恢复、损坏归档拒绝、active 重建、rollback
    与资源清理，U7 P0-2 已关闭。
  - 当前仍有两个生产 P0：CI 漏洞与许可证门禁、H5 发布前
    CSP/依赖锁定/WebCrypto 安全报告；最后由 G4 独立复审作出 Go/No-Go 裁决。
  - 未关闭时阻断 2.0 发布。
- [ ] `IM2-U11` P1 可选纵向切片

...
- 当前立即任务：继续 U8 的真实网络切换、附件设备联调和完整页面巡检；双 iOS 群聊已读详情、禁言状态、成员移除和联系人生命周期均已取得证据。
...
- [ ] `IM2-U8` Flutter 移动 P0 设备验收收口
  ...
  - R1.3 已通过双 iOS 私聊双向已读、群聊已读/未读成员详情、联系人备注/删除/重新申请、真实 UI 建群、群消息双向实时互发、个人禁言/解禁、全体禁言/恢复、成员移除实时退页、管理员任命后对端权限实时刷新、前后台重连和离线文本消息恢复；附件签名/上传失败、图片/文件/语音手动重试、重启恢复和本地文件丢失已由单测覆盖，API contract 已补完整附件链路断言；待补真实网络切换和附件设备联调。

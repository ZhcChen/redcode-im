...
- 当前立即任务：U1、U2 已关闭并推送；唯一 checkpoint 为 `U3.1`，先加固 G1
  cleanup 的外部副作用恢复和 failure/`INT`/`TERM` 边界。
- 当前下一阶段：严格按 `U3 G1 cleanup -> U4 恢复真实性 -> U5 H5 production
...
- [ ] `IM2-U10` E2EE 发布门禁
  ...
  - 当前执行顺序固定为 `U1 -> U2 -> U3 -> U4 -> U5 -> U6 -> U7 -> U8 -> U9`；
    U1（`da3cead2`、`10f0f724`）与 U2（`22a728f9`）已完成，当前只执行
    `U3.1`；后续单元不得提前并行打开。

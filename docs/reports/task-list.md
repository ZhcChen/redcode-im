...
- 当前立即任务：继续 U8 的真实网络切换、文件/语音附件及系统 PHPicker 验收和完整页面巡检；双 iOS 图片附件真实链路、群聊已读详情、禁言状态、成员移除和联系人生命周期均已取得证据。
...
- [ ] `IM2-U8` Flutter 移动 P0 设备验收收口
  ...
  - R1.3 已通过双 iOS 私聊双向已读、群聊已读/未读成员详情、联系人备注/删除/重新申请、真实 UI 建群、群消息双向实时互发、个人禁言/解禁、全体禁言/恢复、成员移除实时退页、管理员任命后对端权限实时刷新、前后台重连、离线文本消息恢复，以及图片附件签名、直传、commit、广播和对端下载；图片/文件/语音失败重试、重启恢复和本地文件丢失已由单测覆盖。待补真实网络切换、文件/语音附件设备联调和系统 PHPicker 人工验收。
  - 待补系统键盘、安全区、长内容、权限首次拒绝/恢复和完整页面巡检。

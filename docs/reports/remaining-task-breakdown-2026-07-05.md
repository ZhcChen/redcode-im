...
任务：
- 已新增 `h5-app/test/e2e/h5-app-smoke.spec.ts` 和 `h5-app/playwright.config.ts`。
- 已新增 `make h5-app.test.e2e`。
- 已覆盖普通账号注册/登录后进入聊天 tab。
- 已覆盖创建/进入群聊，发送消息。
- 已覆盖刷新页面后从本地缓存恢复消息。
- 已覆盖好友申请闭环：搜索用户、发送申请、接受后联系人状态可见。
- 已覆盖群设置关键路径：群设置页打开和置顶切换。

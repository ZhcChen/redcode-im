...
- **自动化测试**：
  - 后端新增 Go（Go 1.25）集成测试（建议放在 `backend/test_go`），覆盖签名校验、白名单校验、大小限制三大场景。
...
## 协作与跟踪
- **责任人**：
  - 后端接口与配置：`backend@liuyang`（主责），`ops@zhangqi`（协同）
  - Flutter 客户端：`flutter@zhangle`
  - 桌面端：`desktop@wuwei`
  - Admin 管理端：`admin@chenxin`
- **每日例会**：每天 10:30 在 `#proj-upload` 进行 15 分钟站会，更新昨日进度、今日计划、阻塞项。
- **周里程碑**：
  - 2025-11-07（周五）Checkpoint：接口伪实现 + 桌面端接入工具初版。

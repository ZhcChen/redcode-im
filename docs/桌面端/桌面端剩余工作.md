...
- **打包脚本**：新增 `desktop/scripts/build-macos.sh` 与 `desktop/scripts/build-linux.sh`。示例：`cd desktop && ./scripts/build-macos.sh arm64`（Apple Silicon）/`./scripts/build-macos.sh intel`（Intel）/`./scripts/build-linux.sh stable-linux`。脚本会自动设置 `VITE_APP_CHANNEL/VITE_APP_VERSION/VITE_APP_BUILD` 并将产物复制到 `dist/releases/<platform>/<channel>`；若需手动调用，可使用 `VITE_APP_CHANNEL=stable-macos-arm64 bun run tauri build --target aarch64-apple-darwin`、`VITE_APP_CHANNEL=stable-linux bun run tauri build --target x86_64-unknown-linux-gnu`、`VITE_APP_CHANNEL=stable-windows-x64 export PATH="/opt/homebrew/opt/llvm/bin:$PATH" && bun run tauri build --runner cargo-xwin --target x86_64-pc-windows-msvc` 等命令。

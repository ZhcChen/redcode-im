...
- **打包脚本**：新增 `desktop/scripts/build-macos.sh`，示例：`cd desktop && ./scripts/build-macos.sh arm64`（Apple Silicon）或 `./scripts/build-macos.sh intel`（Intel）。脚本会自动设置 `VITE_APP_CHANNEL/VITE_APP_VERSION/VITE_APP_BUILD` 并将 `.dmg` 复制到 `dist/releases/macos/<channel>` 目录，便于按渠道上传到 Admin。若需自行调用，可直接使用 `VITE_APP_CHANNEL=stable-macos-arm64 VITE_APP_VERSION=1.0.0 VITE_APP_BUILD=101 bun run tauri build --target aarch64-apple-darwin`（Intel 版本则将渠道与 target 换成 `stable-macos-intel` / `x86_64-apple-darwin`）。

...
- `api/sql/base.sql` 是 **当前完整基线**，可直接初始化空库。
- `api/sql/migrations/` 只存放 **当前基线之后** 的新增迁移。
- 2026-04-09 之前的历史迁移已归档到 `api/sql/migrations_legacy_20260409/`，不再参与默认迁移链路。

## 基本原则

1. **空库初始化优先走 api**：推荐直接启动 api，由 `Database::migrate` 自动执行 `base.sql` 与未完成增量迁移。
2. **增量脚本记录后续变化**：每次结构调整时，在 `api/sql/migrations/` 新增一份 SQL 文件，并同步更新 `api/src/database/mod.rs` 的 `MIGRATIONS` 数组。
...
5. **保持文档同步**：涉及初始化方式、运维流程或 schema 规则变化时，同步更新 `api/sql/README.md` 与相关运维文档。

## 增量脚本要求

- **目录**：`api/sql/migrations/`
...
1. 在 `api/sql/migrations/` 新增增量脚本。
2. 在 `api/src/database/mod.rs` 的 `MIGRATIONS` 数组追加条目。
3. 为新增结构补测试；至少确保空库初始化链路或相关 smoke test 覆盖该变化。
4. 更新相关文档与需求/计划文档。

## 迁移记录与校验

api 会把已执行脚本记录到 `db_migrations`，并保存当前文件 checksum。
...
```sql
-- api/sql/migrations/20260410120000_add_example_column.sql

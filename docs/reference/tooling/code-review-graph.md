...
Code Review Graph（CRG）为 RedCode IM 的 CE 工作流提供可选的代码关系图证据。它只服务于既有的 `plan` 和 `review` 阶段，不新增工作流阶段，不替代 `brainstorm -> plan -> execute -> review -> compound`，也不进入默认测试、提交或推送链。
...
| 场景 | 默认策略 | 主要查询目标 |
|------|----------|--------------|
| 跨 API、原生双端（android-app/ios-app）、H5、Desktop 等模块改动 | 使用 | 公共契约、跨模块调用者、关联测试 |
| 公共符号、类型、接口或状态模型重构 | 使用 | callers、callees、impact radius、测试覆盖 |
...
```bash
codex mcp add code-review-graph-redcode-im -- \
  uvx --from code-review-graph==2.3.7 code-review-graph serve
```

新增 MCP 后需要重启 Codex 会话才能加载工具。注册只修改用户级 Codex MCP 配置，不应生成 Codex hooks 或 Git hooks。
...
```bash
codex mcp get code-review-graph-redcode-im
make crg.status
git check-ignore .code-review-graph/graph.db
make test.all -n | rg -i 'crg|code-review-graph'
make test.live -n | rg -i 'crg|code-review-graph'
```

最后两条命令应无输出。回滚 MCP 时只移除项目专属 server：

```bash
codex mcp remove code-review-graph-redcode-im
uvx --from code-review-graph==2.3.7 code-review-graph uninstall \
  --repo . --keep-user-configs --dry-run
```

检查 dry-run 范围只涉及当前仓库后，才可交互执行不带 `--dry-run` 的卸载。禁止使用 `--all-repos`，也不得移除其他 MCP server、其他仓库图数据或通用用户配置。

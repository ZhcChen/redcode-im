...
- `android-app.test.live` 需要本机 Compose API 已启动，使用 Android APIClient/Auth/Chat/Contacts 数据层覆盖注册、建群、双向文本互发、已读、好友申请/接受和私聊消息 live smoke；该入口强制 `--rerun-tasks`，避免 Gradle 缓存跳过真实联调。
- `android-app.test.interop` 需要本机 Compose API 已启动，会串联 `h5-app.test.live` 与 `android-app.test.live`，作为 H5/API/Android 聊天与好友互通 smoke 入口。

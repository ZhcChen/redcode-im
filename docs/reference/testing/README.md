...
- `android-app.test.live` 需要本机 Compose API 已启动，使用 Android APIClient/Auth/Chat 数据层覆盖注册、建群、双向文本互发和已读 live smoke；该入口强制 `--rerun-tasks`，避免 Gradle 缓存跳过真实联调。
- `android-app.test.interop` 需要本机 Compose API 已启动，会串联 `h5-app.test.live` 与 `android-app.test.live`，作为 H5/API/Android 聊天互通 smoke 入口。

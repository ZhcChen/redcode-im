...
RedCode IM 支持通过 Admin 后台配置服务端是否持久化聊天消息。该配置影响 API、WebSocket、原生双端/H5 客户端和管理后台聊天记录能力。
...
> 当前 API 已完成发送端点强制约束，但原生双端尚未完成密钥生命周期、标准协议加解密、
> 多设备和群聊密钥分发（历史 Flutter 实现仅作协议基线）。因此在正式客户端 E2EE 闭环
> 上线前，不应在生产环境开启 `e2ee`。仅把明文 Base64 编码后调用加密端点不构成端到端
> 加密。

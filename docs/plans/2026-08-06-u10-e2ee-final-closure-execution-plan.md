...
- **唯一恢复点：** `F6.1-C`。F6.1-A/B 已关闭，当前进入全资产 provenance 与构建输入固定整改。
...
| F6.1-C 全资产 provenance 与输入固定 | P1-06 | H5、Android、API 资产均绑定候选 provenance；API base image 固定 digest；最终 Cargo build 使用 `--locked` | `make supply-chain.workflow.test supply-chain.check supply-chain.test` 与 provenance 负例 | in_progress |
...
| --- | --- |
| Active unit | F6 |
| Active checkpoint | F6.1-C 全资产 provenance 与输入固定 |
...
| Immediate action | 执行 F6.1-C：为 Android/API 增加 candidate-bound provenance，并固定 API base image/Cargo 输入 |

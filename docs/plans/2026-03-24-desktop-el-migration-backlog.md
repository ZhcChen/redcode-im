...
- [x] 已完成视频预览控制增强第一刀：预览层现在支持播放 / 暂停、后退 / 前进 5 秒、时间进度展示、静音切换，以及 `Space` / 左右方向键快捷控制；继续复用原生 `<video>` 播放内核，不新增本地服务。
- [x] 已完成视频预览控制增强第二刀：预览层进度条现在支持点击跳转到目标时间点，跳转后会立即刷新播放时间与进度状态。
- [ ] 更完整的预览体验与缓存治理仍未迁移。
...
- [x] 更多细节交互第四刀（媒体预览键盘操作）
- [x] 更多细节交互第五刀（媒体预览内保存当前媒体）
- [x] 更多细节交互第六刀（视频预览控制增强）
- [x] 更多细节交互第七刀（视频预览进度跳转）
- [ ] 更多细节交互（更细视频预览、语音录制增强与更细缓存治理等仍待迁移）
...
- [x] 已补齐 `ws.connect` 失败路径的 app-level 集成测试，并修复握手失败时未向 renderer 发出 `ws.status.updated(disconnected)` 的缺口。

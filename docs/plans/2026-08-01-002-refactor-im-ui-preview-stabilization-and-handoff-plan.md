...
- **后续归属：** `ios-app/`、`app/`、`h5-app/` 等正式实现依据冻结文档分别排期，本计划只交付可执行映射，不直接修改这些模块。
...
- R4. `iPhone 12 Pro`、`iPhone 16 Pro Max` 和 `Pixel 8 Pro` 三种预览必须覆盖安全区、键盘、底部导航和内容裁切检查。
...
- R10. Overlay 必须具备一致的遮罩关闭、`Escape`、焦点恢复和滚动保持行为；仅在现有全量渲染造成可复现问题时才局部更新 DOM。
...
- 直接修改 `ios-app/`、`app/`、`h5-app/`、`desktop/` 或 `admin/`。
...
- KTD2. **问题清单先于代码修复。** U1 先输出带路由、设备、状态和截图证据的审查记录，后续单元只处理确认问题，防止巡检演变成无边界改版。
...
- KTD6. **多端交付采用差异矩阵，不假设一比一复制。** 设计语义和状态契约保持一致，安全区、系统导航、键盘、触觉反馈和平台控件按 iOS、Android、H5 分别标注。
...
flowchart TB
  ...
  F --> G[iOS Android H5 difference matrix]
```
...
- Chrome 的设备模拟不能完全替代真机安全区、键盘和触觉反馈；本轮只建立预览验收基线，正式端仍需各自设备验收。
...
- **Files:** `im-ui-html/assets/app.js`、`im-ui-html/assets/styles.css`、`im-ui-html/assets/mock-data.js`、`im-ui-html/docs/page-map.md`、`docs/reviews/2026-08-01-im-ui-preview-consistency-review.md`。
- **Approach:** 从 `page-map.md` 和实际路由注册生成审查矩阵；逐页检查导航、背景过渡、左右线、Surface、间距、圆角、icon、文字对齐、空/错/加载态及设计说明文案；记录严重度、复现路由和建议归属，只修审查确认项。
...
- **Test Scenarios:** 三种设备上的会话、聊天、联系人、群成员、搜索、朋友圈、设置表单；键盘/安全区；横竖边缘内容；长中文、长英文连续串和混合内容。
...
- **Approach:** 统一页面切换与同页 render 的动画判定；核对 Overlay stacking context、定位边界和底部导航层级；完善短按/长按取消条件、原生 callout 抑制、遮罩/`Escape`/焦点恢复；先保留全量 render，仅对可复现的滚动或焦点问题做局部 DOM 更新。
- **Test Scenarios:** 会话四边触点长按、长按后取消、短按进入详情、菜单动作；Sheet/Popover/Toast 连续切换；表情与功能面板不同高度切换；同页状态操作和真实路由跳转；滚动后打开/关闭 Overlay。
...
- **Requirements:** R7-R12。
- **Dependencies:** U1-U3。
- **Files:** `im-ui-html/package.json`、`im-ui-html/playwright.config.ts`、`im-ui-html/tests/ui-preview-smoke.spec.ts`、`im-ui-html/tests/ui-preview-interactions.spec.ts`、`im-ui-html/README.md`、`docs/reference/testing/README.md`、`Makefile`。
- **Approach:** 使用 `@playwright/test` 启动固定 `8020` 静态服务；配置 iPhone 12 Pro、iPhone 16 Pro Max、Pixel 8 Pro 三个项目；路由测试从页面地图或一份单一清单驱动，禁止在多个测试文件复制路由；失败时保留截图和 trace。
- **Test Scenarios:** 全路由 HTTP/渲染成功、Console error、文档宽度与设备画布溢出、会话短按/长按、动画类切换、底部导航可见、面板高度过渡、朋友圈多图布局。
- **Verification:** `make im-ui.test` 可在干净环境启动静态服务并完成测试；重复执行结果确定；测试结束不残留 `8020` 服务进程。
...
- **Approach:** 标注冻结日期和设计源版本；统一页面、组件、状态、深链和测试覆盖矩阵；在 `platform-handoff.md` 分别记录 iOS、Android/Flutter、H5 的已有实现、缺失能力、平台差异和建议优先级；规定后续新增页面或修改 token 必须更新的文档与回归项。
...
自动化执行前必须按仓库端口规则清理已有 `8020` 占用；Playwright 配置负责启动和回收测试服务。人工视觉检查复用同一浏览器会话，按“定位一次、连续检查、统一验证”执行。

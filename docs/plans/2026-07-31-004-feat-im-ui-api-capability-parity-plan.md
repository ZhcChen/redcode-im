...
- R2. 自己发送的消息必须能进入已读详情，展示已读/未读成员、计数、长名单和空态；未读成员由房间成员列表减去已读列表和发送者计算，不假设已读接口直接返回未读列表。
- R3. 编辑、删除、清空聊天记录等不可逆或高影响操作必须有明确确认状态、处理中状态、成功反馈和失败恢复；删除消息和清空聊天记录都是服务端全局变更，不设计“仅为我删除”；群聊仅群主可清空，私聊任一成员清空都会影响双方服务端历史。
- R4. 会话列表必须提供置顶/取消置顶、通知模式（全部、仅提及、静音）和归档操作，并在归档成功后提供可撤销恢复入口；当前 API 没有归档会话列表查询，因此不得设计成可长期浏览和恢复的归档中心。
...
- R9. 群设置必须按信息架构拆出群资料、群权限、成员、管理员、入群申请、邀请成员、群规则、禁言管理和操作日志；群资料只覆盖 API 现有的群头像、群名称和 `description`，界面可将 `description` 命名为“群介绍”，不得再创建独立群公告字段。
- R10. 所有群治理页面必须提供群主、管理员、普通成员三种确定性角色预览，隐藏或禁用无权限操作并说明原因；群权限覆盖入群审核、成员邀请、成员间加好友限制和人数上限，管理员信息展示 API 返回的角色与权限摘要。
...
- R15. 转让群主、退出群聊和解散群聊必须使用独立确认流程，明确结果和不可撤销影响；群主不提供直接退出入口，必须先转让群主或解散群聊。

**内容、账号与支持**

- R16. 贴纸管理必须覆盖可用贴纸包、搜索、我的贴纸包、添加、单包移除、下载/加载和错误状态；未添加套装只能展示 API 返回的元数据，添加后才能展示子包与表情项，且不得提供 API 不支持的整套批量移除。
- R17. 体验反馈必须从简化 Sheet 升级为正式页面，覆盖分类、正文、联系方式可选项、提交状态和成功结果；分类作为客户端预设前缀合并进 API 的 `content` 字段。
- R18. 个人资料必须表达头像上传和昵称编辑，以及上传中、裁剪/预览、失败和保存状态；用户名和邮箱只读，当前 API 没有个性签名字段，不提供个性签名编辑。
...
- KTD9. **敏感表单只保留在当前内存会话。** 举报截图、反馈联系方式、密码和注销确认不得写入 `localStorage`、URL 或确定性 mock 文件；提交失败保留输入仅指当前页面会话，离开流程后清理。
...
| 会话置顶/通知 | `POST/DELETE /rooms/{room_id}/pin`、`POST /rooms/{room_id}/notification-settings`；通知枚举为全部、仅提及、静音 |
...
| 群成员/资料/头像/退出/转让/解散 | `/rooms/{room_id}`、`/rooms/{room_id}/avatar/*`、`/members`、`/leave`、`/transfer`；群资料只有名称、描述和头像 |
| 群设置/申请/邀请/管理员 | `/settings`、`/join-requests`、`/invitations`、`/admins`；邀请只支持创建和按 ID 响应，没有列表查询 |
| 群禁言/规则/日志 | `/mutes`、`/rules`、`/operation-logs`；操作日志仅支持 `limit/offset` 分页，不支持条件筛选 |
| 贴纸包 | `/emoji-packs/available`、`/search`、`/my`、`/{pack_id}/add`、`/{pack_id}/remove`、`/suites/{suite_id}/packs`；套装内容仅添加后可查且无整套移除 |
...
- **Test Scenarios:** 自己/他人消息动作不同；文本与附件动作不同；编辑后状态可见；删除确认可取消且不出现“仅为我删除”；私聊成员清空时明确双方历史受影响；群主清空；群管理员/普通成员不可清空；转发支持搜索和多选并处理部分目标失败；reaction 仅展示 API 支持的固定集合；已读 0 人、部分已读、全部已读和长名单，未读列表按成员差集计算；`relay_only` 禁用依赖历史的动作。
- **Verification:** 从 `#/chat/:chatId` 连续走通所有动作；独立打开已读/转发深链；检查状态回写到聊天 mock。

### U3. 会话归档与撤销恢复

- **Goal:** 补齐会话置顶、三档通知模式、归档入口和归档后的限时撤销恢复流程，不虚构归档列表能力。
- **Requirements:** R4、R5、R21。
- **Files:** `im-ui-html/assets/app.js`、`im-ui-html/assets/mock-data.js`、`im-ui-html/assets/styles.css`、`im-ui-html/docs/page-map.md`。
- **Patterns:** 复用会话列表和现有 Toast/结果提示，通过侧滑或会话菜单提供动作，在提示中增加明确的撤销命令，不新增归档列表页面。
- **Test Scenarios:** 置顶/取消置顶后排序同步；全部通知/仅提及/静音三态切换；普通会话归档；置顶会话归档；撤销后恢复原位置；归档失败后原位置保持；撤销失败后给出重试；提示超时后不再展示恢复入口。
...
- **Test Scenarios:** 备注编辑为空/超长/保存成功；删除好友取消/确认/失败；举报原因选择、长说明、缺少截图校验、1/多张截图、上传中、提交失败和成功；截图与举报正文不进入持久化状态；删除后已有会话仍可见但关系标识变化。
- **Verification:** 从资料页进入所有流程并正确返回；举报深链可独立评审。

### U5. 群成员、管理员与群生命周期

- **Goal:** 补齐群名称/介绍/头像编辑、成员列表、角色管理、退出、转让和解散页面状态。
- **Requirements:** R9-R11、R15、R21。
- **Files:** `im-ui-html/assets/app.js`、`im-ui-html/assets/mock-data.js`、`im-ui-html/assets/styles.css`、`im-ui-html/docs/page-map.md`。
- **Patterns:** 复用群目录成员列表、联系人选择器和统一确认组件。
- **Test Scenarios:** 群头像预览/上传失败；群名称和群介绍编辑；页面不存在独立群公告字段；普通成员只读；长成员列表搜索；群主任命/移除管理员及权限摘要；管理员移除普通成员；不能移除群主；转让目标选择；群主只显示转让或解散，管理员/普通成员可退出；退出和解散使用不同影响文案；处理失败后保留当前页。
- **Verification:** owner/admin/member 三种角色深链截图，动作矩阵与 API 权限一致。

### U6. 群申请、邀请、规则、禁言与日志

- **Goal:** 完成群权限设置及其余群治理页面族和角色状态。
- **Requirements:** R9-R10、R12-R14、R21。
- **Files:** `im-ui-html/assets/app.js`、`im-ui-html/assets/mock-data.js`、`im-ui-html/assets/styles.css`、`im-ui-html/docs/page-map.md`。
- **Patterns:** 复用分段筛选、状态标签、列表行、表单和时间线样式。
- **Test Scenarios:** 入群审核、成员邀请、成员间加好友限制和人数上限；申请待处理/通过/拒绝；邀请成员搜索、多选和创建结果；通过已知邀请 ID 展示待响应/接受/拒绝/过期；不存在邀请列表入口；规则增改删和空态；全员禁言开关；单人定时禁言和解除；当前用户被禁言；操作日志分页时间线且不存在 API 不支持的条件筛选；普通成员无写权限。
- **Verification:** 各子页固定路由可打开，角色切换后操作显隐和状态说明正确。

### U7. 贴纸中心与聊天入口

- **Goal:** 补齐贴纸商店、我的贴纸、受 API 数据边界约束的套装状态及聊天选择面板映射。
...
- **Test Scenarios:** 可用/我的/搜索结果；添加和单包移除；未添加套装只显示名称、图标和描述；添加套装后显示子包与表情项；页面不存在整套批量移除；下载中、下载失败、空搜索；聊天面板显示已添加贴纸并保持面板高度过渡。
...
- **Test Scenarios:** 反馈分类和长正文；失败时仅在当前会话保留输入；联系方式、密码和注销确认不进入持久化状态；头像选择/预览/上传失败；昵称未修改禁用保存；用户名和邮箱只读；页面不存在个性签名编辑；密码校验错误；注销二次确认、处理中和失败；页面不出现 SMS 重置或短信登录入口。
...
| 敏感状态 | U4、U8 | 检查 `localStorage`、URL、mock 数据和页面退出后的内存状态 | 举报、反馈、密码和注销确认数据未被持久化 |
...
- D10. 设计源不出现归档会话列表、邀请列表、操作日志条件筛选、独立群公告、个性签名编辑、未添加套装内容预览、整套贴纸批量移除、无截图举报，也不把反馈分类映射为独立 API 字段等当前 API 无法直接支撑的契约。
- D11. 举报截图和正文、反馈联系方式、密码及注销确认不写入 `localStorage`、URL 或 mock 文件，离开对应流程后不保留。

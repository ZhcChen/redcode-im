...
- **把 `#/spec` 升格为第一入口**：它不再只是设计展示页，而是 2.0 的规范源。
...
### Unit 1：收紧 `#/spec`，把规范页做成 2.0 真正入口

**目标：** 先把 token、密度、布局、组件状态、动效原则讲清楚。
...
- **先堆页面、后补规范**：会导致风格再次发散。规避方式：`#/spec` 必须先完成。
...
- 每个里程碑都要保证推荐评审路径可复现。
- `README`、`#/spec`、业务路由和 `docs/` 说明必须互相一致。

## 下一步

1. 直接从 Unit 1 开始，先升级 `#/spec`。
2. 然后进入移动端核心页面重构。

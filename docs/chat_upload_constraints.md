...
### 3. 扩展能力决策流程
- **需求确认**：产品在 2025-11-12 前提供语音通话、位置分享初步需求文档，若延期则视为本迭代不纳入范围。
- **技术评估**：桌面与移动端联合输出评估，重点关注：
  - 语音通话：是否复用现有 RTC 服务、带宽与权限策略。
  - 位置分享：是否需要地图 SDK、后端数据存储与过期策略。
- **里程碑输出**：在 2025-11-14 之前将评估结论与资源需求同步到 `docs/product/roadmap.md`，并更新本文件的下一步状态。

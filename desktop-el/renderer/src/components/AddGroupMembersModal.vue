...
const selectedFriends = computed(() =>
  selectedMemberIds.value
    ...
    .filter((friend): friend is GroupFriendOption => Boolean(friend)),
);
...
const handleSubmit = () => {
  if (!selectedMemberIds.value.length) {
    validationMessage.value = "请至少选择 1 位好友";
    return;
  }
      ...
      <section class="create-group-modal__panel">
        <header class="create-group-modal__header">
          <div>
            ...
            <h2>添加群成员</h2>
            <small>从当前好友列表中选择要加入该群的成员。</small>
          </div>
          <button
            type="button"
          ...
          <section class="create-group-modal__picker">
            ...
            <div class="create-group-modal__picker-grid">
              <div class="create-group-modal__list">
                ...
                >
                  <strong>{{
                    props.friends.length ? "暂无匹配好友" : "暂无可添加好友"
                  }}</strong>
                  <p>
                    {{
                      props.friends.length
                        ? "换个关键词试试"
                        : "当前好友都已在群内，或你还没有可邀请的好友"
                    }}
                  </p>
                </div>

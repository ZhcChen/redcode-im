...
`android-app` 是 RedCode IM 2.0 的原生 Android 客户端主线（Kotlin + Jetpack
Compose），与 `ios-app/` 一起替代已弃用的 Flutter `app/`。

## 当前定位

- Flutter `app/` 已弃用（GPU 兼容层问题，2026-08-04 决策），本模块与
  `ios-app/` 是 2.0 唯一移动客户端主线。
- 版本与最低系统：2.0.0；`minSdk 24`（Android 7.0）；Gradle 使用 JDK 21 运行，
  字节码 target 17。
...
当前模块自带 Gradle wrapper（Gradle 8.12），Makefile 通过 `android-app/gradlew`
运行，JDK 使用 21：

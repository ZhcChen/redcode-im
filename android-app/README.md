...
- OkHttp WebSocket JSON client 已接入，支持鉴权、房间订阅、ping 和重连基线
...
- WebSocket JVM 单测覆盖 URL 规范化、auth/ping、join/leave、typing guard、服务端错误、断开清理和失败重连；protobuf 二进制帧与增量消息落库在后续切片补齐。

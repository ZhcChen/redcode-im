...
然后再按 `api/src/database/mod.rs` 的 `MIGRATIONS` 顺序补执行后续 active migration。
...
`db_migrations` 由 api 自动维护，当前字段包括：

- `name`：迁移文件名
- `checksum`：当前文件内容的 SHA-256 指纹
- `applied_at`：执行时间

已记录 migration 在启动时会校验 checksum；如果文件被人改过，api 会直接拒绝继续启动。

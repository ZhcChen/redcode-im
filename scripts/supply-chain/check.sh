...
while IFS=$'\t' read -r module lockfile scanner scope; do
  test -s "$ROOT_DIR/$lockfile" || { echo "missing or empty lockfile: $lockfile" >&2; exit 1; }
  if [ "$scanner" = "swift-osv" ]; then
    bun "$ROOT_DIR/scripts/supply-chain/scan-swift.ts" \
      "$ROOT_DIR/$lockfile" "$POLICY" "$REPORTS_DIR/$module.json" "$SBOM_DIR/$module.cdx.json" "$COMMIT"
    continue
  fi
  ...
  jq -e '.bomFormat == "CycloneDX" and .specVersion == "1.5" and (.components | length > 0)' \
    "$SBOM_DIR/$module.cdx.json" >/dev/null
...
bun "$ROOT_DIR/scripts/supply-chain/evaluate.ts" \
  "$POLICY" "$EXCEPTIONS" "$REPORTS_DIR" "$OUTPUT_DIR/summary.json" "$COMMIT"

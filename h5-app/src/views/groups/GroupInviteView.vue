...
const submit = async () => {
  ...
  try {
    if (mode.value === 'invite') {
      const count = appEnv.useMockData
        ? selected.value.length
        : (await roomService.createInvitations(roomId.value, selected.value, message.value)).length;
      notice.value = `已发送 ${count} 条群邀请`;
      message.value = '';
    } else {
...
<template>
  <main class="contact-page app-phone-frame">
    ...
    <section class="contact-page__content">
      ...
      <div v-if="canManage" class="contact-page__tabs"><button type="button" :class="{ active: mode === 'add' }" @click="mode = 'add'">直接添加</button><button type="button" :class="{ active: mode === 'invite' }" @click="mode = 'invite'">发送邀请</button></div>
      <label v-if="canManage && mode === 'invite'" class="contact-page__field">邀请附言（可选）<textarea v-model="message" class="rc-focus-ring" maxlength="200" placeholder="填写群邀请附言" /></label>

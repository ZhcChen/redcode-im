...
export function new_protocol_state() {
    const ret = wasm.new_protocol_state();
 ...
 * @returns {boolean}
 */
export function validate_protocol_state(encoded) {
    const ptr0 = passArray8ToWasm0(encoded, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.validate_protocol_state(ptr0, len0);
    return ret !== 0;
}
function __wbg_get_imports() {
    ...
    return {
        __proto__: null,
        "./redcode_e2ee_core_bg.js": import0,
    };
}
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('redcode_e2ee_core_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();

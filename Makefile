...
.PHONY: help status install.all test.all test.live tests.all dev.up dev.down dev.logs crg.build crg.update crg.status crg.review \
	...
	app.install app.run app.check app.test app.test.unit app.test.scripts app.test.api-paths app.test.core app.test.chat app.test.widgets app.test.features app.test.integration.smoke app.test.integration.network app.test.integration.auth app.test.integration.contract app.test.integration.device app.test.integration.device.auth app.test.integration.device.contract app.test.integration.device.reverse app.test.integration.device.auth.reverse app.test.ios-device-acceptance app.test.ios-permission-acceptance app.test.patrol.harness app.test.patrol.login app.test.patrol.dual app.test.patrol.cross app.test.patrol.cross-offline app.test.patrol.group app.test.patrol.group-mute app.test.patrol.group-member-removal app.test.patrol.image-attachment app.test.patrol.rich-attachment app.test.patrol.network app.test.patrol.contact app.test.patrol.offline app.test.patrol.pages app.test.patrol.layout app.test.patrol.permission app.build.android app.build.ios app.proto \
...
app.test.ios-permission-acceptance: ## 执行 iOS Simulator 首次照片拒绝、设置恢复与 PHPicker 验收
	@test -n "$(APP_IOS_ACCEPTANCE_DEVICE)" -a -n "$(APP_IOS_ACCEPTANCE_ACCOUNT)" -a -n "$(APP_IOS_ACCEPTANCE_PEER_ACCOUNT)" -a -n "$(APP_IOS_ACCEPTANCE_PASSWORD)" || (echo "缺少 APP_IOS_ACCEPTANCE_DEVICE/ACCOUNT/PEER_ACCOUNT/PASSWORD" >&2; exit 2)
	@xcrun simctl bootstatus "$(APP_IOS_ACCEPTANCE_DEVICE)" -b
	...
	@cd "$(APP_DIR)/ios" && result="../build/ios-permission-acceptance-$$(date +%s).xcresult" && dart_defines="$$(printf '%s' 'API_BASE_URL=http://127.0.0.1:8010' | base64),$$(printf '%s' 'WS_URL=ws://127.0.0.1:8010/ws' | base64)" && REDCODE_TEST_ACCOUNT="$(APP_IOS_ACCEPTANCE_ACCOUNT)" REDCODE_TEST_PEER_ACCOUNT="$(APP_IOS_ACCEPTANCE_PEER_ACCOUNT)" REDCODE_TEST_PASSWORD="$(APP_IOS_ACCEPTANCE_PASSWORD)" xcodebuild test -quiet -workspace Runner.xcworkspace -scheme Runner -testPlan TestPlan -destination "platform=iOS Simulator,id=$(APP_IOS_ACCEPTANCE_DEVICE)" -only-testing:RunnerUITests/RedCodeDeviceAcceptanceTests/testPhotoDenialAndSettingsRecovery -resultBundlePath "$$result" DART_DEFINES="$$dart_defines" 'GCC_PREPROCESSOR_DEFINITIONS=$$(inherited) CLEAR_PERMISSIONS=0 FULL_ISOLATION=0' && echo "iOS 权限验收证据: $(APP_DIR)/$${result#../}"

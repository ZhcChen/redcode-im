...
ANDROID_GRADLEW := $(ANDROID_APP_DIR)/gradlew
ANDROID_GRADLE ?= $(ANDROID_GRADLEW)
...
.PHONY: help status install.all test.all test.live tests.all dev.up dev.down dev.logs crg.build crg.update crg.status crg.review supply-chain.check supply-chain.test supply-chain.workflow.test release.reliability.test release.environment.test e2ee.provenance.test e2ee-core.wasm-builder.test \
	...
	h5-app.install h5-app.up h5-app.down h5-app.wait h5-app.logs h5-app.build h5-app.release.build h5-app.release.check h5-app.release.test h5-app.release.browser-signal.test h5-app.release.candidate.audit h5-app.release.candidate.test h5-app.release.secure-state.audit h5-app.check h5-app.test h5-app.test.unit h5-app.test.live h5-app.test.e2ee.live h5-app.test.e2ee.restore-live h5-app.test.e2e e2ee.evidence.verify e2ee.evidence.test im-ui.install im-ui.test im-ui.test.visual \
...
e2ee.cross-client.live: ## 受控执行 Android/iOS/H5 E2EE 真实跨端联调并恢复 plaintext
	@"$(ROOT_DIR)/tests/scripts/run-e2ee-cross-client-live.sh"

e2ee.cross-client.recovery.test: ## 验证 E2EE live 失败及信号中断后的 runtime 恢复
	@"$(ROOT_DIR)/tests/scripts/test-e2ee-live-runtime-recovery.sh"

e2ee.cross-client.isolated.test: ## 验证隔离 restore identity、SSH tunnel 与 cleanup 合同
	@"$(ROOT_DIR)/tests/scripts/test-e2ee-isolated-live-guard.sh"

e2ee.backup-drill.test: ## 验证 E2EE 备份演练 failure/signal/幂等 cleanup
	@"$(ROOT_DIR)/tests/scripts/test-e2ee-backup-rollout-drill.sh"

e2ee.restore-compose.test: ## 验证 E2EE 独立恢复 API/PG/Redis 的网络与端口隔离合同
	@"$(ROOT_DIR)/tests/scripts/test-e2ee-restore-compose.sh"
...
e2ee.restore-boundary.test: ## 验证 restore DB/Redis/log/Push/RustFS 边界扫描合同
	@"$(ROOT_DIR)/tests/scripts/test-e2ee-restore-boundary-scan.sh"
...
e2ee.evidence.verify: ## 离线校验 U10 E2EE 持久脱敏 evidence
	@$(BUN) "$(ROOT_DIR)/scripts/e2ee-evidence/verify.ts"

e2ee.evidence.test: ## 执行 U10 E2EE evidence 生成、篡改与敏感扫描测试
	@$(BUN) "$(ROOT_DIR)/tests/scripts/test-e2ee-evidence.ts"
...
tests.tooling: ## 执行仓库级 tooling 守护测试
	...
	@$(MAKE) release.reliability.test
	@$(MAKE) release.environment.test
	@$(MAKE) e2ee.provenance.test
	@$(MAKE) e2ee.evidence.verify
	@$(MAKE) e2ee.evidence.test
	@$(MAKE) e2ee-core.wasm-builder.test
	@$(MAKE) h5-app.release.test
	@$(MAKE) h5-app.release.candidate.test
...
release.reliability.test: ## 执行 Release create-only 与 API artifact cleanup 正负测试
	@"$(ROOT_DIR)/tests/scripts/test-release-reliability.sh"

release.environment.test: ## 验证 production-release environment policy 采集与 fail-closed
	@"$(ROOT_DIR)/tests/scripts/test-production-environment.sh"

e2ee.provenance.test: ## 验证四类 release sidecar provenance 的离线验签契约
	@$(BUN) "$(ROOT_DIR)/tests/scripts/test-e2ee-provenance.ts"

e2ee-core.wasm-builder.test: ## 执行 canonical WASM platform/version fail-closed 测试
	@"$(ROOT_DIR)/tests/scripts/test-e2ee-wasm-builder.sh"
